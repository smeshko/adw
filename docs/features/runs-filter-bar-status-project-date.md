...
- **`hx-include="#filter-form"` for combined params**: All filter controls live inside a `<form id="filter-form">`. Each control uses `hx-include="#filter-form"` so that every change sends ALL current filter values plus the sort param. This ensures filters are cumulative and the URL stays in sync.

- **Hidden sort input**: A `<input type="hidden" name="sort" value="{{ sort }}">` inside the filter bar form preserves the current sort order when filters change. The page param is omitted to reset to page 1.
...
- Sort order is preserved when filters change via the hidden sort input
- The "Clear all" link only appears when at least one filter is active
- Empty state message includes a suggestion to adjust filters
