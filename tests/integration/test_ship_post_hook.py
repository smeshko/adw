...
"""
        ...
        assert "Merge Reason: Deployment successful, all checks passed" in result.stdout
...
"""
        env = {
            ...
            "ADW_PR_URL": "https://github.com/o/r/pull/50",
            "PATH": "/usr/bin:/bin",
        }
...
"""
        env = {
            ...
            "ADW_FEATURE": "Test",
            "ADW_ARTIFACTS_DIR": str(artifacts_dir),
            "ADW_PR_URL": "https://github.com/o/r/pull/60",
            "PATH": "/usr/bin:/bin",
        }
...
"""
        env = {
            ...
            "PATH": "/usr/bin:/bin",
            # No ADW_PR_URL and no PR_NUMBER in output
        }
    ...
    def test_pr_number_from_output_overrides_pr_url(
        self, post_hook_path: Path, tmp_path: Path
    ) -> None:
        """Test that PR_NUMBER from LLM output is used over ADW_PR_URL."""
        artifacts_dir = tmp_path / "artifacts"
        artifacts_dir.mkdir()
...
"""
        env = {
            ...
            "ADW_FEATURE": "Test",
            "ADW_ARTIFACTS_DIR": str(artifacts_dir),
            "ADW_PR_URL": "https://github.com/o/r/pull/123",  # Should be overridden by LLM output
            "PATH": "/usr/bin:/bin",
        }
    ...
    def test_pr_number_from_adw_pr_url(self, post_hook_path: Path) -> None:
        """Without PR_NUMBER in the output, the number comes from ADW_PR_URL."""
        llm_output = """
DEPLOYMENT_STATUS: SUCCESS
PR_MERGE_APPROVED: true
MERGE_REASON: Success
"""
        env = {
            "ADW_LLM_OUTPUT": llm_output,
            "ADW_PHASE": "ship",
            "ADW_PR_URL": "https://github.com/o/r/pull/77",
            "PATH": "/usr/bin:/bin",
        }
        ...
        assert "PR Number: 77" in result.stdout
        assert "gh pr merge 77 --squash" in result.stdout

    def test_non_numeric_pr_url_is_ignored(self, post_hook_path: Path) -> None:
        """An ADW_PR_URL that does not end in a PR number yields no number."""
        llm_output = """
DEPLOYMENT_STATUS: SUCCESS
PR_MERGE_APPROVED: true
MERGE_REASON: Success
"""
        env = {
            "ADW_LLM_OUTPUT": llm_output,
            "ADW_PHASE": "ship",
            "ADW_PR_URL": "https://example.com/not-a-pr",
            "PATH": "/usr/bin:/bin",
        }
    ...
    def test_no_status_or_task_files_written(
        self, post_hook_path: Path, tmp_path: Path, fake_gh: "FakeGh"
    ) -> None:
        """An approved, merged run writes merge_record.json and no dead files."""
        artifacts_dir = tmp_path / "artifacts"
        artifacts_dir.mkdir()
        llm_output = """
DEPLOYMENT_STATUS: SUCCESS
...
PR_NUMBER: 88
"""
        fake_gh.reply(stdout="")
        env = {
            "ADW_LLM_OUTPUT": llm_output,
            "ADW_PHASE": "ship",
            "ADW_ARTIFACTS_DIR": str(artifacts_dir),
            "ADW_TASK_ID": "TASK-123",
            "FAKE_GH_DIR": str(fake_gh.state_dir),
            "PATH": f"{fake_gh.state_dir / 'bin'}:/usr/bin:/bin",
        }
        ...
        assert (artifacts_dir / "merge_record.json").exists()
        assert not (artifacts_dir / "ship_status.json").exists()
        assert not (artifacts_dir / "task_update_request.json").exists()
