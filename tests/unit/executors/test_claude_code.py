...
class TestRealTimeStreaming:
    ...
    def test_outputs_to_live_stream_when_configured(self, tmp_path) -> None:
        ...
        # Verify live.log was written to
...
class TestStreamEventExtraction:
    """Tests for _extract_stream_event() method (Task 2).

    This method parses stream-json lines and returns structured StreamEvent
    objects for real-time handling of text, tool starts, and tool results.
    """
    ...
    def test_extract_text_delta(self, executor: ClaudeCodeExecutor) -> None:
        """Should extract text from content_block_delta with text_delta."""
        import json
        from adw.models.llm import StreamEventType
        ...
        event = executor._extract_stream_event(line)

        assert event is not None
        assert event.event_type == StreamEventType.TEXT
        assert event.content == "Hello world"

    def test_extract_tool_start(self, executor: ClaudeCodeExecutor) -> None:
        """Should extract tool start from content_block_start with tool_use."""
        import json
        from adw.models.llm import StreamEventType
    ...
    def test_extract_tool_result(self, executor: ClaudeCodeExecutor) -> None:
        """Should extract tool result from tool_result message."""
        import json
        from adw.models.llm import StreamEventType
        ...
        assert event is not None
        assert event.event_type == StreamEventType.TOOL_RESULT
        assert event.tool_id == "toolu_01abc"
        assert event.tool_output == "File contents here..."
        assert event.is_error is False

    def test_extract_tool_result_error(self, executor: ClaudeCodeExecutor) -> None:
        """Should extract tool result with is_error=True."""
        import json
        from adw.models.llm import StreamEventType
    ...
    def test_returns_none_for_invalid_json(self, executor: ClaudeCodeExecutor) -> None:
        """Should return None for invalid JSON."""
        event = executor._extract_stream_event("not valid json")
        assert event is None

    def test_returns_none_for_non_dict_json(self, executor: ClaudeCodeExecutor) -> None:
        """Should return None for non-dict JSON (e.g., number)."""
        event = executor._extract_stream_event("42")
        assert event is None

    def test_returns_none_for_unrecognized_message_type(
        self, executor: ClaudeCodeExecutor
    ) -> None:
        """Should return None for unrecognized message types."""
        import json
    ...
    def test_returns_none_for_empty_text_delta(
        self, executor: ClaudeCodeExecutor
    ) -> None:
        """Should return None for text_delta with empty text."""
        import json
...
class TestRealTimeToolStreaming:
    """Tests for real-time tool call streaming (AC1, AC3).

    Verifies that tool calls appear in live.log immediately with
    correct timestamps, not batched at the end.
    """

    @pytest.fixture
    def executor_with_live_stream(self, tmp_path):
        """Create executor with live_stream transport."""
        from adw.logging.live_stream import LiveStreamTransport
        ...
        live_stream = LiveStreamTransport(live_log_path)
        executor = ClaudeCodeExecutor(config, live_stream=live_stream)
        return executor, live_stream, live_log_path

    def test_tool_call_streamed_in_real_time(
        self, executor_with_live_stream, tmp_path
    ) -> None:
        """Tool calls should appear in live.log during streaming, not at end.

        This verifies AC1: Real-time tool calls with current timestamps.
        """
        import json

        executor, live_stream, live_log_path = executor_with_live_stream

        # Simulate stream-json output with tool_use
        ...
        content = live_log_path.read_text()
        # Tool call should appear in live.log
        assert "[TOOL]" in content
        assert "Read" in content
        # Tool result box should appear
    ...
    def test_no_duplicate_tool_calls(
        ...
        """Tool calls should appear exactly once, not both during streaming AND at end.

        This verifies AC3: No duplicate tool calls.
        """
        import json

        executor, live_stream, live_log_path = executor_with_live_stream

        # Simulate stream with tool_use in assistant message (which also gets parsed later)
        tool_start = json.dumps({
            "type": "content_block_start",
            "content_block": {
                "type": "tool_use",
        ...
        content = live_log_path.read_text()
        # Count occurrences of the tool call
        tool_count = content.count("[TOOL]")
        # Should appear exactly once (from real-time streaming, not duplicated at end)
        assert tool_count == 1, f"Expected 1 tool call, found {tool_count}"


class TestStreamEventDataclass:
    """Tests for StreamEvent dataclass (Task 1)."""

    def test_stream_event_text_creation(self) -> None:
        """StreamEvent should be creatable with TEXT type."""
        from adw.models.llm import StreamEvent, StreamEventType
    ...
    def test_stream_event_tool_start_creation(self) -> None:
        """StreamEvent should be creatable with TOOL_START type."""
        from adw.models.llm import StreamEvent, StreamEventType
        ...
        assert event.tool_name == "Read"
        assert event.tool_id == "toolu_01abc"
        assert event.tool_input == {"file_path": "/src/main.py"}

    def test_stream_event_tool_result_creation(self) -> None:
        """StreamEvent should be creatable with TOOL_RESULT type."""
        from adw.models.llm import StreamEvent, StreamEventType
    ...
    def test_stream_event_type_enum_values(self) -> None:
        """StreamEventType should have correct enum values."""
        from adw.models.llm import StreamEventType
