...
class TestValidationPhaseReportIntegration:
    """Integration tests for ValidationPhase report generation."""

    def test_phase_generates_report(self, tmp_path: Path) -> None:
        """Phase generates report when configured with run_id."""
        from adw.validation.loop_controller import ExitReason
        ...
        # Update loop controller counts manually
        phase.loop_controller.state.issues_deferred = 1
        phase.loop_controller.state.current_iteration = 3

        report = phase.generate_report(issues, ExitReason.MAX_ITERATIONS)

        assert report is not None
        assert report.run_id == run_id
        assert report.issues_deferred == 1
        assert report.exit_reason == "MAX_ITERATIONS"

    def test_phase_no_report_without_run_id(self) -> None:
        """Phase returns None if no run_id configured."""
        from adw.validation.phase import ValidationPhase

        phase = ValidationPhase()
        report = phase.generate_report([])

        assert report is None

    def test_phase_stores_last_report(self, tmp_path: Path) -> None:
        """Phase stores last generated report."""
        from adw.validation.phase import ValidationPhase
        ...
        assert phase.last_report is not None
        assert phase.last_report.run_id == run_id

    def test_phase_pr_section_empty_without_report(self) -> None:
        """PR section returns empty without report."""
        from adw.validation.phase import ValidationPhase

        phase = ValidationPhase()
        assert phase.get_pr_section() == ""

    def test_phase_report_saves_files(self, tmp_path: Path) -> None:
        """Report generation saves MD and JSON files."""
        from adw.validation.phase import ValidationPhase
        ...
        artifacts_dir = runs_dir / run_id / "artifacts" / "validation"
