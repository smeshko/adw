...
def run_summary_step(
    ...
    try:
        ...
        return {
            "confirmed": True,
            "action": "complete",
            "files_created": [str(adw_dir / path) for path in files.keys()],
        }
    except ConfigWriteError as e:
        console.print(f"\n[red]Error writing configuration:[/] {e}")
        console.print("[dim]No files were created.[/]")
        return {"confirmed": True, "action": "error", "files_created": [], "error": str(e)}
...
def generate_summary_panel(state: WizardState) -> Panel:
    ...
    lines.append(f"  Name: {basics.get('project_name', basics.get('language', 'unknown'))}")
    ...
    if llm_retry.get("customized", False):
        ...
        lines.append(f"[cyan]LLM Retry:[/] Custom ({max_retries} retries, {base_delay}s base)")
    else:
...
def generate_project_yaml(state: WizardState) -> str:
    ...
    if security.get("allow_dangerous", False) or security.get("blocked_patterns") or security.get("blocked_env_files"):
...
def generate_phase_configs(state: WizardState) -> dict[str, str]:
    ...
    for phase in customized_phases:
        ...
        if phase_config:
            ...
            if config:
                header = f"# Phase configuration for {phase}\n# Generated by ADW Init Wizard\n\n"
                yaml_content = yaml.dump(config, default_flow_style=False, sort_keys=False)
